...
export function buildObsidianVault(input: ObsidianInput = {}): {
  ...
  archive.push(
    '## 按时更新',
    '',
    '日常拿数走本机，不经过呼和浩特服务器，也不连 GitHub。',
    '路径是：腾讯行情 → 本机分析 → 重写看台、结论、变化。',
    'GitHub 只在改程序时才需要。网页要今日数据，仍须从 Mac 跑 ship-from-mac.sh。',
    '',
    '本机装一次时钟，之后打开 Obsidian 看 [[看台]] 即可：',
    '',
    '    ./scripts/install-mac-clock.sh install',
    '',
    '交易日北京时间 09:20 盘前、15:10 盘后自动重算。本机一律不归档。',
    '电脑开着才会跑。睡着会错过这一次。醒来后若看台日期不是今天，跑：',
    '',
    '    ./scripts/install-mac-clock.sh run-now',
    '',
    '两条路，不要同时当两本账：',
    '',
    '- 本机复跑：拉真实行情 → 程序分析 → 重写看台。不连 GitHub。',
  ...
  notes.push({
    ...
    body: [
      ...
      '',
      '日常拿数走本机，不经过呼和浩特服务器，也不连 GitHub。',
      '装一次之后，打开 [[看台]] 就能读最后结果。不要每天在代码目录敲命令。',
      '',
      '在代码目录执行一次：',
      '',
      '    ./scripts/install-mac-clock.sh install',
      '',
      '默认笔记库是代码目录下的 鸿鹄理财。若库在别处，先设 OBSIDIAN_VAULT。',
      '交易日 09:20 盘前、15:10 盘后会自动重算。电脑开着才会跑。睡着会错过。',
      '醒来后若看台日期不是今天：',
      '',
      '    ./scripts/install-mac-clock.sh run-now',
      '',
      '现在立刻手跑一次（不等人、不连 GitHub）：',
      '',
