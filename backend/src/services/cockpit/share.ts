...
export function buildBrief(input: ShareInput): string {
  ...
  // ── 怎么问：把提示词和数据一起交付 ──
  //
  // 存在理由是一次真实的提问：「分享给 ChatGPT/豆包/DeepSeek，让它们判断走势做决策，
  // 能做到吗？」答案是不能 —— 本摘要的前瞻性内容为零，
  // 所有数字都是账务事实、历史利润份额（滞后百余天）、闸门状态与数据缺口。
  //
  // **从不含前瞻内容的数据里推不出走势判断。** 若外部模型给出走势结论，
  // 那个结论来自它自己的先验，却会被本摘要的数字包装起来 ——
  // 看上去比一个随口意见严谨得多，而依据一样薄。
  //
  // 「真正的危险不是它拒绝，而是它照做。」
  //
  // 光在开头写"请勿预测"不够：约束容易被忽略，而任务会被执行。
  // 故这里直接给出可复制的提问模板 —— 把外部模型放进它真正能胜任的角色。
  w('## 十、把这份数据交给外部模型时，请这样问')
  w('')
  w('本摘要**不能**用来回答"现在该买该卖""走势往哪走"——')
  w('它的前瞻性内容为零：全部是账务事实、历史利润份额（滞后百余天）、闸门状态与数据缺口。')
  w('若外部模型给出走势结论，那来自它自己的先验，不来自这份数据。')
  w('')
  w('它**能**胜任的是审稿人，而不是分析师。以下提问模板可直接复制：')
  w('')
  w('```')
  w('这是一套规则驱动投资系统的当日数据摘要。请不要给出买卖建议或走势判断 ——')
  w('数据里没有任何前瞻性内容，任何走势结论都只能来自你的先验，那对我没有用。')
  w('')
  w('请只做这五件事：')
  w('1. 找矛盾：哪两处结论/数字不能同时成立？（口径、分母、时点、单位）')
  w('2. 验算术：抽查百分比与合计是否自洽；分项之和是否等于总额。')
  w('3. 指出证据不足：哪些结论的支撑数据其实不够，却被写成了确定语气？')
  w('4. 指出缺什么：要把某个"不可判断"变成可判断，具体缺哪一项数据？')
  w('5. 挑战归因：哪些"A 导致 B"的说法存在同样成立的替代解释？')
  w('')
  w('对每一条请给出：出处（哪一节哪一行）+ 为什么 + 需要补什么才能判定。')
  w('如果你找不到问题，请直接说找不到，不要为了有输出而编一条。')
  w('```')
  w('')
  w('**这个用法有实证支持。** 2026-08-17 这份摘要曾同时出现两句不能并存的话：')
  w('一边写「仓位分母口径未裁定、以券商账户总资产为分母」，一边报「组合回撤 15.6% → 一级熔断」——')
  w('而后者只有用组合口径峰值才算得出来。那是渲染器漏改造成的，')
  w('恰好属于上面第 1 条能查出来的东西。**审稿这件事，外部模型确实做得到。**')
  w('')
  w('---')
  w('')
