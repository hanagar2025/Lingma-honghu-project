...
export function renderDashboardHtml(input: HtmlInput): string {
  const {
    ...
    powerChain, portfolioDefense, ownershipPhilosophy, aiActTwoPool, capexLadder, aiFourActs, dalioPressureTest, aiFinancingQuality, aiPhaseTwo, lookout,
  } = input
  const h = d.headline
  ...
  if (capexLadder) {
    w(`</div><div class=card><h2>AI 资本开支产业链迁移体检 —— 逐层检查走到哪一层，不预测谁涨</h2>`)
    w(`<pre class=plain>${esc(renderCapexLadder())}</pre>`)
  }
  ...
  if (aiFinancingQuality) {
    w(`</div><div class=card><h2>AI 融资质量观察 —— 信用周期待验证，发债不是泡沫</h2>`)
    w(`<pre class=plain>${esc(renderAiFinancingQuality())}</pre>`)
  }
