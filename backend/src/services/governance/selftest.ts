...
// "更聪明"，把叙事台账的评分接进候选池排序 —— 那时文档还在，约束已经没了。
{
  ...
  ok('决策层与研究层源码都能读到（读不到等于检查空转）',
    decision.length === 17 && research.length === RESEARCH_MODULES.length,
    `决策 ${decision.length}/17，研究 ${research.length}/${RESEARCH_MODULES.length}`)
  ...
  ok('特征文本表已登记且覆盖关键词',
    ...
    && ['命题 A', '因果强度', '替代解释', '超级周期', 'H-AI', '组合防守层', 'P-01', 'P2-01', 'F-01', 'H-FQ', 'T-01', 'H-DL', 'A-01', 'H-PC', 'S-01', 'H-S2', 'M-01', 'H-MR', 'B-01', 'H-BQ']
  ...
  ok('字段名表覆盖当前研究模块导出的关键字段',
    ['hypotheses', 'attribution', 'alternatives', 'link2', 'causal', 'portfolioDefense', 'ownershipPhilosophy', 'aiPhaseTwo', 'aiFinancingQuality', 'dalioPressureTest', 'aiFourActs', 'aiActTwoPool', 'capexLadder', 'macroRiskLights', 'marketStructureWatch', 'moneyCockpit']
