...
export const PEAK_HISTORY: PeakRecord[] = [
  ...
  },
  {
    ...
    reliableFrom: null,
    note:
      '组合口径（持仓 + 账内现金 + 账户外股票现金）峰值 630 万。'
      + '委员会 2026-08-15 裁定采用情形 B：账户峰值 430 万发生时，'
      + '那 200 万已明确属于股票投资资金，故组合峰值 = 430 + 200 = 630 万。'
      + 'peakDate 仍为 null —— 峰值出现的具体日期未从日志考据，不猜；'
      + '它不影响回撤计算，只影响将来复盘时能否定位那一天。'
      + 'reliableFrom 仍为 null —— 期间该笔资金的逐日余额没有记录，'
      + '故 630 万只可用作「峰值」这一个点，不可用于重建整条组合口径净值曲线。',
  },
]
