...
### 口令强度：判据是"剩余熵"，不是"含不含某个词"

口令必须经得起**离线**爆破：密文文件谁都能下载，然后在自己机器上慢慢试，
没有次数限制，也不会像在线登录那样锁账号。

实测（PBKDF2-SHA256 60 万轮，8 张 RTX4090 约 3.3 万次/秒）：

| 口令 | 离线爆破耗时 |
|---|---|
| `honghu2026` | 不到 1 秒 |
| `honghu@wealth` | 15 秒 |
| `honghu2026-mist-loom-42x` | 250 年 |
| `honghu-青瓦-灯塔-47` | 3251 年 |

**「honghu」本身没有问题** —— 问题是除它之外没有别的东西。
所以检查的不是"含不含品牌名"，而是**去掉可猜成分（品牌名、年份、顺序串、
分隔符）后还剩多少熵**，下限 36 比特（约等于 8 个随机小写字母）。
上表后两个都含 `honghu`，都能通过。

### 中文口令：可以用，而且更划算

一个常用汉字约 11.5 比特，一个小写字母只有 4.7 —— **两个汉字抵五个字母**。
`青瓦灯塔鸿鹄远` 七个字约 58 比特剩余熵，离线爆破需约 10 万年。

已实测加密与前端解密全链路（纯中文、中英混合、含数字、含全角标点、含 emoji 均通过）。

两个跨设备的坑，都实测确认过：

- **只用汉字与数字，别用标点。** 全角「，」与半角「,」是不同字符 ——
  手机与电脑的输入法可能给出不同的那一个，于是同一个口令在手机上解不开。
- **别用带音标的拉丁字母**（`café` 之类）。它们有 NFC/NFD 两种编码形式，
  不同系统输入的字节可能不同。纯汉字不受影响（汉字不分解）。

另外有一条硬性要求：至少 6 个字符。这**不是**强度要求，而是防成语 ——
「一帆风顺」按每字 11.5 比特算得 46 比特，但它是词典里的一个条目，
实际只有几万种可能（约 16 比特），比特估算在这种情况下会严重高估。
拼凑无关的字（`青瓦灯塔鸿鹄远`）才配得上那个比特数。

看候选口令：

```bash
node scripts/check-passphrase.mjs --suggest
```

它给的是**真随机**串，不假装好记。因为好记与高熵是互斥的 ——
任何背得下来的东西，其生成规则一旦公开（本仓库就是公开的），
真实搜索空间就是那个规则的组合数。这里踩过一次：先前版本从 24 词表里拼装，
估算器报 53 比特，真实组合只有约 5 万种（16 比特，一秒即破），差三十多个数量级。

### 推荐做法：不要背，交给钥匙串

挑一个真随机口令，第一次解锁时让 Safari 存入**系统钥匙串**，之后用 Face ID 自动填入。
解锁表单已按规范写好（`form` + `autoComplete="current-password"`），Safari 会主动提示保存。
另在纸上抄一份放家里作为备份。要记的东西是解锁手机，不是这串字符。

这与"本页不保存口令"不矛盾：钥匙串是操作系统的加密存储、由 Face ID 保护，
和我们自己把明文塞进 `localStorage` 是两件完全不同的事。

### 让好记的口令也安全：把页面放在猜不到的路径下

如果确实要用背得下来的口令，就补上另一层 —— 别让人拿到密文：

```bash
BASE_PATH=/k7m3x9p2/ DEPLOY_HOST=39.104.86.200 ./scripts/deploy-ecs.sh
```

页面变成 `https://hhwealth.cc/k7m3x9p2/`，根目录返回 404。
自动扫描器找不到那个密文文件，**离线爆破就无从开始** ——
而离线爆破是弱口令唯一的死穴。加个手机书签，日常使用没有任何区别。

注意这是"让攻击者拿不到密文"，不是"密文更难破"。两层都要有：
路径挡住机器扫描，口令挡住拿到文件的人。

确实要用弱口令且不加路径保护：`TIOS_ALLOW_WEAK=1`，
脚本会打印预估爆破耗时并要求手工输入确认语。
...
线上页面是**静态快照**。它不会自己变 —— 只在发布时更新一次。
解锁页顶部显示「快照生成时间」，任何时候都能确认自己看的是哪一份。

装两个定时任务（只需一次）：

```bash
security add-generic-password -a "$USER" -s tios-deploy-passphrase -w   # 存口令，只需一次
...
其他命令：`status` 看是否加载与线上是哪个交易日、`run-now` 立即跑一次、`uninstall` 卸载。

### 休市日自动跳过，且不用节假日日历

判据是「行情源给出的最新K线日期是否等于今天」。日历会因调休和临时休市过期，
而「最新K线是哪天」永远是当下的事实。线上已是当日数据时也跳过（`FORCE=1` 可强制覆盖）。

### 为什么定时任务在 Mac 上而不在服务器上

服务器 always-on，看着更合适。但**加密快照必须在持有口令的机器上生成**，
而整套加密的前提正是「服务端自己也解不开」。口令放到服务器上，那句话立刻变成假的：
能生成密文就能解开密文。而服务器暴露在公网、有历史遗留应用、root 可能被他人接管，
你的 Mac 不是。所以生成留在 Mac，服务器只当静态托管。

代价是 Mac 得醒着并联网。选 launchd 而非 crontab 就是为此 ——
睡眠期间错过的任务，launchd 会在唤醒后补跑一次，cron 不会。

口令从 macOS 钥匙串取（`security find-generic-password`），
不落盘、不进 plist、不进环境变量文件。plist 是明文且会被 Time Machine 备份，绝不能放秘密。
